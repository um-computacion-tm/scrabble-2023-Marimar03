...
class TestBagTiles(unittest.TestCase):
    def test_initialize_tiles(self):
        bag = BagTiles()
        tiles = bag.tiles
        tile_count = bag.tile_count
        tile_scores = bag.tile_scores

        # Verificar que haya 100 fichas en total
        self.assertEqual(len(tiles), 100)

        # Verificar que haya 2 comodines
        self.assertEqual(tile_count['?'], 2)

        # Verificar cantidad y valor de las letras según las reglas del Scrabble
        letter_counts = {
        ...
        for letter, count in letter_counts.items():
            self.assertEqual(tile_count[letter], count)
            self.assertEqual(tile_scores[letter], self.calculate_scrabble_score(letter))

    def calculate_scrabble_score(self, letter):
        scrabble_scores = {
    ...
    @patch('random.shuffle') #lo hizo el profe
